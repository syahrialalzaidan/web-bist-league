    ...
    return (
        <section>
            <div className={`h-20 ${css.outer}`}>
                ...
                {/* Logo Bist League */}
                <button >
                    <Link href='/'><img src="/images/logo.png" alt="logo" className={css.logo} /></Link>
                </button>

                <div className={`${css.rightNavbar}`}>
                    {/* Button */}
                    <button className={`${css.navbarItem} ${props.page === 'Home' ? css.buttonActive : ''}`} ><Link href='/' className={``}>Home</Link></button>
                    <button className={`${css.navbarItem} `} ><Link href='/competetion' className={`${props.page === 'Competetion' ? css.buttonActive : ''}`}>Competetion</Link></button>
                    <button className={`${css.navbarItem} ${props.page === 'Bootcamp' ? css.buttonActive : ''}`} ><Link href='/bootcamp'>Bootcamp</Link></button>
                    <button className={`${css.navbarItem} ${props.page === 'Webinar' ? css.buttonActive : ''}`} ><Link href='/webinar'>Webinar</Link></button>
                    <button className={`${css.navbarItem} ${props.page === 'Mini Challenge' ? css.buttonActive : ''}`} ><Link href='/minichallenge'>Mini Challenge</Link></button>

                    {props.username === null ?
                        <button className={`${css.navbarItem}`} ><Link href='#'>Register</Link></button>
                        :

                        <div className={css.relative}>
                            <div className={css.topleftusername}>
                                <p className={`${css.hiUsername}`}>Hi, <span>{props.username}!</span></p>
                                <div className={css.personAndDropdown}>
                                    <button onClick={() => handleDropdownToggle()}>
                                        <svg className={css.BsPersonCircle} stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" color="#379392" height="1.8rem" width="1.8rem" xmlns="http://www.w3.org/2000/svg"><path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"></path><path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1z"></path></svg>
                                    </button>

                                    <button onClick={() => handleDropdownToggle()}>
                                        <svg className={css.dropdownIcon} width="15" height="15" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div className={`${css.dropdown} ${dropdownToggleClicked ? css.dropDownActive : ''}`}>
                    ...
                    {/* Navbar Burger */}
                    <img src="/images/navbarToggle.png" alt="navbarToggle" className={css.navbarToggle} onClick={() => handleNavbarToggle()} />
                </div>

            </div>

            {/* Overlay */}
            <div className={`${css.overlay} ${navbarToggleClicked ? css.show : ''}`} id="overlay">
                <ul>
                    <li className={css.backButtonSection}>
                        <button><img src="./images/back-arrow.png" alt="" className={css.backButton} onClick={() => handleBackButton()} /><Link href="#" className={css.backText} onClick={() => handleBackButton()}>Back</Link></button>
                    </li>
                    ...
                    {props.username === null ?
                        <li className="flex justify-center"><button className={`${css.registerButton}`} ><Link href='#'>Register</Link></button></li>
                        :
                        <li>
                    ...
                    <div>
                        ...
                        <li className={`${css.profileLi} ${dropdownLiToggleClicked ? css.show : ''}`}>
                            <button>
                                Dashboard
                            </button>
                        </li>
